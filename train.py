...
#                                     lambda data: decorate_with_diffs(data, 2.0), lambda x: x * 2)
# (name, preprocess, d_input_func) = ("Data and diffs",
#                                     lambda data: decorate_with_diffs(data, 1.0), lambda x: x * 2)
(name, preprocess, d_input_func) = ("Only 4 moments",
                                    lambda data: get_moments(data), lambda x: 4)
...
def get_moments(d):
    ...
    final = torch.cat((mean.reshape(1,), std.reshape(
        1,), skews.reshape(1,), kurtoses.reshape(1,)))
    return final
    ...
    for epoch in range(num_epochs):
        ...
        for g_index in range(g_steps):
            ...
            g_error = criterion(dg_fake_decision, Variable(
                torch.ones([1])))  # Train G to pretend it's genuine
